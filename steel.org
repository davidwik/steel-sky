...
** No bell!
Just recently emacs started to beep for no reason.
...
*steel/rust-format-on-save-hook*
Format cleanup when save.
#+BEGIN_SRC emacs-lisp :tangle yes

  (defun steel/rust-format-on-save-hook ()
    "Format Rust buffer before saving."
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (use-package flycheck
    ...
    (setq
     ...
     flycheck-disabled-checkers '(rust-ts rust-cargo)
     )
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (use-package org
    ...
    ;; Remap org src blocks to tree-sitter modes
    (setq org-src-lang-modes
          (append
           '(("js" . js-ts)     ;; js → js-ts-mode
             ("javascript" . js-ts)
             ("jsx" . js-ts))  ;; jsx → tsx-ts-mode
           org-src-lang-modes))
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (use-package helm-posframe
    :straight (helm-posframe :type git :host github :repo "tumashu/helm-posframe")
    :ensure t
    :config
    (setq
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (add-hook 'python-ts-mode-hook
            (lambda ()
              (set-fill-column 80)
              (if (not (getenv "NO_RUFF_AUTOFORMAT"))
                  (ruff-format-on-save-mode t)
                (message "RUFF AUTOFORMAT DISABLED")
                )
              )
            )
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (use-package rust-ts-mode
    ...
    (rust-ts-mode . lsp)
    (rust-ts-mode . steel/rust-format-on-save-hook)
    )
#+END_SRC
...
Overrides in order to use treesitter (which is faster) instead of the traditional modes.

#+BEGIN_SRC emacs-lisp :tangle yes

  ;; We would like to use the python-ts-mode instead of the traditional.
  (add-to-list
   'auto-mode-alist
   '("\\.py\\'" . python-ts-mode))
